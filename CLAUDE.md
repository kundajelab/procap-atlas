...
- **`src/bpnet/`** — BPNet deep learning model training and evaluation:
  ...
  - `attribute/attribute_bpnet.py` — Computes DeepLIFT/SHAP attributions across all folds for a trained BPNet, averaging fold attributions genome-wide. Accepts the same `--background NAME:RATIO` / `--model-dir` interface. `--head` selects profile or count head; `--ohe` saves one-hot-encoded sequences. Output: `attributions/bpnet/{model_dir_name}_{head}.npz`.
