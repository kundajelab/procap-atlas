...
- **`src/bpnet/`** — BPNet deep learning model training and evaluation:
  ...
  - `attribute/launch.py` — Submits SLURM jobs for attributions, one per (experiment, head) pair. Skips experiments with fewer than `--min-reads` total reads (default: 10M), incomplete training (any fold model missing), or existing output. `--head` is repeatable (default: `profile`). Logs to `logs/bpnet_attr/`.
