# File taken from ProCapNet repo:
# https://github.com/kundajelab/ProCapNet/blob/5664daa8c123d143275faae7c5385c99fa76bd5a/src/1_process_data/_merge_uni_bi_peaks.py
...
def merge_uni_bi_peaks(uni_peaks_filename, bi_peaks_filename):
    ...
    with gzip.open(uni_peaks_filename) as uni_peaks_file:
        for line in uni_peaks_file:
            ...
            if strand == "+":
                uni_peaks.append(
                    (chrom, start, end, strand, ".", "Unidirectional", summit, ".")
                )
            else:
                uni_peaks.append(
                    (chrom, start, end, strand, ".", "Unidirectional", ".", summit)
