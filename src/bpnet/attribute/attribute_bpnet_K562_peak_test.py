"""Compute DeepLIFT/SHAP attributions for ENCSR220XSM models on ENCSR261KBX peaks/vice versa.

Usage:
    python src/bpnet/attribute/attribute_bpnet_K562_peak_test.py --model ENCSR261KBX --exp ENCSR220XSM --ohe
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="experiment accession ID (e.g. ENCSR220XSM)",
    )
    parser.add_argument(
        "--exp",
        type=str,
    ...
    if args.exp not in experiments:
        print(f"Error: {args.exp} not found in config", file=sys.stderr)
        sys.exit(1)
    ...
    model_dir = REPO_ROOT / "models" / "bpnet" / args.model
    model_paths = [
        model_dir / f"{args.model}.fold{fold}.torch" for fold in range(n_folds)
    ]
    ...
    if args.ohe:
        ohe_path = out_dir / f"{args.exp}_ohe.npz"
    ...
    out_path = out_dir / f"{args.model}_{args.exp}_{args.head}.npz"
    out_dir.mkdir(parents=True, exist_ok=True)
