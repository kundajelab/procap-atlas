...
def main():
    ...
    for exp_id in experiments:
        ...
        for head in heads:
            ...
            if not (
                hits_seqlet_filtered.exists()
                or hits_confidence_filtered.exists()
                or hits_dedensified.exists()
                or hits_unique.exists()
            ):
                skipped_missing += 1
                continue

            seqlets_tsv = hits_dir / "report" / "seqlets.tsv"
            if not seqlets_tsv.exists():
