#!/usr/bin/env python3
"""Submit SLURM jobs to drop hits below what their own discovery seqlets
would support.

Reads experiment IDs from configs/experiment_config.yaml and submits one
sbatch job per (experiment, head) pair via filter_by_seqlet_importance.py.
Run after hitcall/launch_low_confidence_hits.py and before
hitcall/launch_report.py, which prefers this step's output
(hits_seqlet_filtered.tsv) when present.
...
hits_seqlet_filtered.tsv already exists, if report/seqlets.tsv is missing,
or if neither hits_confidence_filtered.tsv, hits_dedensified.tsv, nor
hits_unique.tsv exists. This step does not use a GPU, so it runs as its own
cheap CPU-only launcher, like launch_report.py.

Usage:
    python src/bpnet/hitcall/launch_seqlet_importance.py                    # submit all experiments, profile head
    python src/bpnet/hitcall/launch_seqlet_importance.py --dry-run           # print sbatch scripts without submitting
    python src/bpnet/hitcall/launch_seqlet_importance.py --head profile --head count
    python src/bpnet/hitcall/launch_seqlet_importance.py --min-reads 20000000
    python src/bpnet/hitcall/launch_seqlet_importance.py --min-trim-len 6  # match hitcall/launch.py's floor
"""
...
FILTER_SCRIPT = (
    REPO_ROOT / "src" / "bpnet" / "hitcall" / "filter_by_seqlet_importance.py"
)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="extra arguments forwarded to filter_by_seqlet_importance.py (e.g. '--percentile-multiplier 0.75')",
    )
    parser.add_argument(
        "--min-trim-len",
        type=int,
    ...
    log_dir = REPO_ROOT / "logs" / "bpnet_hitcall_seqlet_importance"
    ...
    for exp_id in experiments:
        ...
        for head in heads:
            ...
            if not (
                compressed_io.exists(hits_confidence_filtered)
            ...
            if compressed_io.exists(hits_seqlet_filtered):
                skipped_done += 1
                continue

            job_name = f"bpnet_hitcall_seqlet_importance_{exp_id}_{head}{suffix}"
            filter_cmd = (
    ...
    print(
        ...
        f"report/seqlets.tsv, skipped {skipped_done} already filtered "
        f"({total} total)"
    )
