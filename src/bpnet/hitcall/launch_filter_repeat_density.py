#!/usr/bin/env python3
"""Submit SLURM jobs to drop dense same-motif repeat clusters from Fi-NeMo
hits.

Reads experiment IDs from configs/experiment_config.yaml and submits one
sbatch job per (experiment, head) pair via filter_repeat_density.py. Must run
before hitcall/launch_report.py so report_bpnet.py picks up
hits_dedensified.tsv and recomputes cwm_similarity against the cleaned hits.

Jobs are skipped if hits_dedensified.tsv already exists or if hits_unique.tsv
is missing (run call_hits_bpnet.py/hitcall/launch.py first). This step does
not use a GPU, so it runs as its own cheap CPU-only launcher, like
launch_report.py.

Usage:
    python src/bpnet/hitcall/launch_filter_repeat_density.py                    # submit all experiments, profile head
    python src/bpnet/hitcall/launch_filter_repeat_density.py --dry-run           # print sbatch scripts without submitting
    python src/bpnet/hitcall/launch_filter_repeat_density.py --head profile --head count
    python src/bpnet/hitcall/launch_filter_repeat_density.py --min-reads 20000000
    python src/bpnet/hitcall/launch_filter_repeat_density.py --min-trim-len 6  # match hitcall/launch.py's floor
"""
...
FILTER_SCRIPT = REPO_ROOT / "src" / "bpnet" / "hitcall" / "filter_repeat_density.py"
...
def main():
    ...
    log_dir = REPO_ROOT / "logs" / "bpnet_hitcall_filter_repeat_density"
    ...
    for exp_id in experiments:
        ...
        for head in heads:
            ...
            if compressed_io.exists(hits_dedensified):
                skipped_done += 1
                continue

            job_name = f"bpnet_hitcall_filter_repeat_density_{exp_id}_{head}{suffix}"
            filter_cmd = (
    ...
    print(
        ...
        f"hits_unique.tsv, skipped {skipped_done} already filtered ({total} total)"
    )
